      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="mb-24 flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-10">
          <div className="max-w-3xl">
            ...
            {/* JUDUL H2: UPDATED STYLE (GLOWING BLUE) */}
          ...
          <div className="lg:pb-4">
            <p className="text-gray-400 text-lg max-w-md mb-6 leading-relaxed">
              Kami membangun solusi skala enterprise yang digunakan oleh
              industri, private sector, instansi pemerintah dan universitas top
              di Indonesia.
            </p>
            <Link href="/portfolio">
              <button className="group flex items-center gap-3 text-base font-bold text-white transition-all">
                ...
                <ArrowUpRight
                  size={20}
                  className="text-blue-500 group-hover:text-white group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform"
                />
              </button>
            </Link>
          </div>
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${
          isFeatured
            ? "lg:col-span-3 md:col-span-2 min-h-[500px] flex flex-col lg:flex-row"
            : "col-span-1 flex flex-col min-h-[450px]"
        }
      `}
    >
      {/* 1. VISUAL AREA */}
      <div
        className={`
            ...
            ${
              isFeatured
                ? "w-full lg:w-5/12 bg-gradient-to-br from-[#0f172a] to-[#020617]"
                : "h-64 bg-[#0f172a]"
            }
        `}
      >
        {/* Background Effects */}
        <div
          className={`absolute inset-0 bg-gradient-to-br ${project.color} opacity-[0.08] group-hover:opacity-20 transition-opacity duration-700`}
        />
        <div
          className={`absolute -top-20 -left-20 w-64 h-64 bg-gradient-to-br ${project.color} blur-[100px] opacity-20 rounded-full group-hover:scale-125 transition-transform duration-1000`}
        />
      ...
      <div
        className={`
            ...
            ${
              isFeatured
                ? "w-full lg:w-7/12 border-t lg:border-t-0 lg:border-l border-white/5"
                : "border-t border-white/5 flex-grow"
            }
        `}
      >
        {/* CLIENT BADGE */}
        <div className="mb-8">
          <div className="inline-flex items-center gap-4 bg-white/[0.03] border border-white/10 p-2 pr-6 rounded-full group-hover:bg-white/[0.08] group-hover:border-white/20 transition-all duration-300">
            <div
              className={`w-10 h-10 rounded-full bg-gradient-to-br ${project.color} flex items-center justify-center shadow-lg`}
            >
        ...
        <div className="mb-6">
          <span
            className={`text-xs font-bold tracking-[0.2em] uppercase text-transparent bg-clip-text bg-gradient-to-r ${project.color} mb-3 block`}
          >
            {project.category}
          </span>
          <h3
            className={`font-black text-white leading-[1.1] group-hover:text-blue-100 transition-colors ${
        ...
        <div className="mt-auto pt-6 border-t border-white/5 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t, i) => (
              <span
                key={i}
                className="px-4 py-2 rounded-lg bg-[#1e293b]/50 border border-white/10 text-xs font-mono text-gray-300 font-semibold group-hover:border-white/30 transition-colors"
              >
                {t}
              </span>
            ))}
